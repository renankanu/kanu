  ...
  return (
    <div className="relative z-10 flex-1 rounded-2xl border border-[#20293A]">
      ...
      <div className="-mt-0.5">
        <div className="ml-4 mr-2 mt-4 flex items-center gap-6 rounded-full bg-slate-800 dark:bg-slate-800">
          <div className="-m-2">{icon}</div>
          <div className="py-2 pr-4 text-sm font-bold text-slate-300 dark:text-slate-300">
            {title}
          </div>
        </div>
      </div>
      <div className="p-4 pl-12 text-sm text-slate-400 dark:text-slate-400">
        {desc}
      </div>
    </div>
  )
